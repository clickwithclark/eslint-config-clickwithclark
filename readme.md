...
NOTICE: This is a customization of wes bos' repo you can find [here](https://github.com/wesbos/eslint-config-wesbos)
...
- You can see all the [rules here](https://github.com/clickwithclark/eslint-config-clickwithclark/blob/master/.eslintrc.js) - these generally abide by the code written in my courses. You are very welcome to overwrite any of these settings, or just fork the entire thing to create your own.
...
```
npm install eslint-config-clickwithclark
```
...
For TypeScript projects, use `clickwithclark/typescript`.

```json
{
  "extends": ["clickwithclark/typescript"]
}
```

TypeScript users will also need a `tsconfig.json` file in their project. An empty object (`{}`) or [my base](https://github.com/clickwithclark/dotfiles/blob/master/tsconfig.json) will do!
...
Same instructions as above, just make sure you extend `clickwithclark/typescript` instead of just `clickwithclark`.
